...
def DAE_fit(X_data,epochs):
    ...
    patch_size=(32,32)
    slidingDis=8
    data_input_Patch,Patch_Idx=GetPatch(data,patch_size,slidingDis)
    data_input=Patch2TrainData(data_input_Patch,patch_size)
    print(data_max)
    print(data_min)
    ...
    data_output=DAE_mode.predict(data_input)
    print(np.max(data_output))
    print(np.min(data_output))
    data_output=ReConstruct(X_data.shape,patch_size,Patch_Idx,data_output,data_max,data_min)
    print(np.max(data_output))
    print(np.min(data_output))
    return data_output
